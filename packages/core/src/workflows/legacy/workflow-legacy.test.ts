...
describe('LegacyWorkflow', async () => {
  ...
  describe('Loops', () => {
    it('should run an until loop', async () => {
      ...
      const counterWorkflow = new LegacyWorkflow<[typeof incrementStep, typeof finalStep]>({
  ...
  describe('Suspend and Resume', () => {
    ...
    it('should handle basic suspend and resume flow', async () => {
      ...
      const promptEvalWorkflow = new LegacyWorkflow({
        name: 'test-workflow',
  ...
  describe('Nested workflows', () => {
    it('should be able to nest workflows', async () => {
      ...
      const wfA = new LegacyWorkflow({ name: 'nested-workflow-a' })
        .step(startStep)
        .then(otherStep)
        .then(finalStep)
        .commit();
      const wfB = new LegacyWorkflow({ name: 'nested-workflow-b' }).step(startStep).then(finalStep).commit();
      counterWorkflow
        .step(wfA)
        .step(wfB)
        .after([wfA, wfB])
    ...
    it('should be able to nest workflows with conditions', async () => {
      ...
      const wfA = new LegacyWorkflow({ name: 'nested-workflow-a' })
        ...
        .then(finalStep)
        .commit();
      const wfB = new LegacyWorkflow({ name: 'nested-workflow-b' })
        .step(startStep)
        .if(async () => false)
        .then(otherStep)
        .else()
    ...
    describe('new if else branching syntax with nested workflows', () => {
      ...
      it('should execute nested else and if-branch', async () => {
        ...
        const wfB = new LegacyWorkflow({ name: 'nested-workflow-b' })
          ...
          .if(
            async () => true,
            new LegacyWorkflow({ name: 'nested-workflow-c' }).step(otherStep).commit(),
            new LegacyWorkflow({ name: 'nested-workflow-d' }).step(otherStep).commit(),
          )
          .then(finalStep)
          .commit();
    ...
    describe('LegacyWorkflow results', () => {
