...
export const providerMapToIcon = {
  openai: (
    <svg className="h-3 w-3" viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" fill="white">
...
const NameCell = ({ row }: { row: any }) => {
  return (
    <EntryCell
      icon={<AgentIcon />}
      name={
        <Link className="w-full space-y-0" to={`/agents/${row.original.id}/chat`}>
          {row.original.name}
        </Link>
      }
...
export const agentsTableColumns: DataTableProps<any, any>['columns'] = [
  {
    id: 'name',
    header: 'Name',
    cell: NameCell,
  ...
  },
  {
    ...
    size: 160,
    cell: ({ row }: { row: any }) => {
      const provider = row.original.provider.split('.')[0];

      return (
        <Cell>
          <Badge variant="default" icon={providerMapToIcon[provider as keyof typeof providerMapToIcon] ?? null}>
