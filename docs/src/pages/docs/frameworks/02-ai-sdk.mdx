...
> **Gotcha**: When using `useChat` with agent memory functionality, make sure to check out the [Agent Memory section](/docs/agents/01-agent-memory#usechat) for important implementation details.
...
](/docs/agents/02-adding-tools#vercel-ai-sdk-tool-format) for more details.


### Client-side tool calling

Mastra leverages AI SDK's tool calling, so what applies in AI SDK applies here still. 
[Agent Tools](/docs/agents/02-adding-tools) in Mastra are 100% percent compatible with AI SDK tools.
