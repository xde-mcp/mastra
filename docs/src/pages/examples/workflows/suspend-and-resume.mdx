...
const stepTwo = new Step({
  ...
  execute: async ({ context, suspend }) => {
    ...
    const secondValue = context.resumeData?.secondValue ?? 0;
...
const promptAgent = new Step({
  ...
  execute: async ({ context, suspend }) => {
    ...
    const guidance = context.resumeData?.guidance;
...
const improveResponse = new Step({
  ...
  execute: async ({ context, suspend }) => {
    ...
    const improvedContent = context.resumeData?.improvedContent;
    const resumeAttempts = context.resumeData?.resumeAttempts ?? 0;
