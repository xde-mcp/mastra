...
デフォルトでは、`Memory`インスタンスは現在のMemoryスレッドから[最新の40メッセージ](../../reference/memory/Memory.mdx)を各新規リクエストに含めます。これにより、エージェントに即時の会話コンテキストが提供されます。
...
デフォルトでは、[Mastraメインインスタンス](https://mastra.ai/reference/core/mastra-class#initialization)に提供されたのと同じストレージを使用します。
