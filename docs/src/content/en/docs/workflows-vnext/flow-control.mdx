...
The output from each step is automatically passed to the next step if schemas match. If the schemas don't match, you can use the [`map` function](/docs/workflows-vnext/input-data-mapping.mdx) to transform the output to the expected schema.
