...
<Tabs items={["npm", "pnpm", "yarn", "bun"]}>
  <Tabs.Tab>```bash copy npm install dotenv --save-dev ```</Tabs.Tab>
  <Tabs.Tab>```bash copy pnpm add dotenv --save-dev ```</Tabs.Tab>
  <Tabs.Tab>```bash copy yarn add dotenv --dev ```</Tabs.Tab>
  <Tabs.Tab>```bash copy bun add dotenv --dev ```</Tabs.Tab>
</Tabs>
