...
<Tabs items={["npm", "yarn", "pnpm"]}>
  <Tabs.Tab>```bash copy npm install @mastra/agui@alpha ```</Tabs.Tab>
  <Tabs.Tab>```bash copy yarn add @mastra/agui@alpha ```</Tabs.Tab>
  <Tabs.Tab>```bash copy pnpm add @mastra/agui@alpha ```</Tabs.Tab>
</Tabs>
