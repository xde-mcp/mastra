...
Similarly, Mastra `Agent` instances can also be exposed as tools via `MCPServer`. This allows other MCP clients to interact with your agents by "asking" them questions. Each agent provided in the `MCPServer` configuration will be converted into a tool named `ask_<agentKey>`, using the agent's `description` property.

This allows others to use your tools and agents without needing direct access to your codebase.

### Using `MCPServer`

You initialize `MCPServer` with a name, version, and the Mastra tools and/or agents you want to share.
...
For an agent to be exposed as a tool, it must have a non-empty `description` string. If the description is missing or empty, `MCPServer` will throw an error during initialization.
