...
<Tabs items={["mcp.run", "Composio.dev", "Smithery.ai", "Ampersand"]}>
  ...
  <Tabs.Tab>
    ...
    [Ampersand](https://withampersand.com?utm_source=mastra-docs) offers a single multi-tenant [MCP Server](https://www.npmjs.com/package/@amp-labs/mcp-server) that allows you to connect your agent to 150+ B2B Integrations. 
    

    ```typescript

    // MCPClient with Ampersand MCP Server with SSE
    export const mcp = new MCPClient({
        servers: {
        "@amp-labs/mcp-server": {
          "url": `https://mcp.withampersand.com/sse?${new URLSearchParams({
