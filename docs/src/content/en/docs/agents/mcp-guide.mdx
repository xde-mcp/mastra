...
The `MCPClient` class provides a way to manage multiple tool servers in your Mastra applications without managing multiple MCP clients. The `MastraMCPClient` used internally by `MCPConfiguration` now automatically detects the transport type based on your server configuration:
...
The `MCPClient` class provides two ways to access MCP tools, each suited for different use cases:
...
For each Profile, you can copy/paste unique, signed URLs into your `MCPClient` like this:
...
- Learn more about [MCPClient](/reference/tools/mcp-client)
