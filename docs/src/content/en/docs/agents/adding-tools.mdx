...
We support dependency injection for tools and workflows. You can pass a runtimeContext directly to your `generate` or `stream` function calls, or inject it using [server middleware](/docs/deployment/server#Middleware).
